...
function NowPlayingBanner() {
    ...
    useEffect(() => {
        const searchReq = async () => {
            ...
            setNowPlaying(nowReq.results[0]);
        }
        searchReq();
    }, []);
    ...
    return (
        <div className="w-full relative h-96 overflow-hidden px-8">
            <div class="absolute w-full h-full opacity-70 top-0 left-0 z-10 bg-gradient-to-r from-primary via-primary"></div>
                <div class="absolute z-20 flex flex-col items-left w-1/2 justify-center h-full px-10">
                    ...
                    <div class="text-white text-xl mt-4 font-medium line-clamp-7">{nowPlaying.overview}</div>
