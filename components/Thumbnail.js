...
const Thumbnail = forwardRef(({result}, ref) => {
    ...
    return (
        <div ref={ref} className='p-2 group cursor-pointer transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50' onMouseEnter={e => { setHoverInfo({display: 'block'});}} onMouseLeave={e => { setHoverInfo({display: 'none'})}}>
            ...
            <p style={hoverInfo} className="absolute bottom-20 p-4 group-hover:font-bold">{result.description || result.overview}</p>
            <p style={hoverInfo} className="absolute bottom-12 p-4 group-hover:font-bold">{result.release_date || result.first_air_date}</p>
