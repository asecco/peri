    ...
    return (
        <div ref={ref} className='p-2 group cursor-pointer transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50' onMouseEnter={e => { setHoverInfo({display: 'block'});}} onMouseLeave={e => { setHoverInfo({display: 'none'})}}>
            ...
            <div style={hoverInfo} layout='responsive' className="absolute bottom-20 p-4 group-hover:font-bold text-base">
                <p className='line-clamp-9'>{result.description || result.overview}</p>
            </div>
            <p style={hoverInfo} layout='responsive' className="absolute bottom-12 p-4 group-hover:font-bold text-xl text-center">{result.release_date || result.first_air_date}</p>
