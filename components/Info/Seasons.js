...
function Seasons({ result, id }) {
    return (
        <div className='p-2 group cursor-pointer transition duration-200 ease-in transform sm:hover:scale-105 '>
            <Link href={`/tv/${id}/season/${result.season_number}`}>
                <Image placeholder='blur' blurDataURL={blurUrl} className='group-hover:opacity-50 rounded-lg' src={`${BASE_URL}${result.poster_path}`} alt='' height={1920} width={1280}/>
