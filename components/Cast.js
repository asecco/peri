    ...
    return (
        <div>
            <div onClick={routeQuery} className="group text-center px-4 md:px-6 xl:px-14 2xl:px-12 py-6 mx-auto transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50 cursor-pointer">
                ...
                <p className="text-2xl font-bold transition-all duration-100 ease-in-out text-white group-hover:text-red-400 md:line-clamp-2">{member.name}</p>
                <p className="my-2 text-md font-bold transition-all duration-100 ease-in-out text-white md:line-clamp-2">{member.character}</p>
            </div>
        </div>
    );
}
