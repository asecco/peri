    ...
    return (
        <div className='p-2 group cursor-pointer transition duration-200 ease-in transform sm:hover:scale-105 hover:z-40'>
            <a href={`/info?type=${result.media_type || 'movie'}&id=${result.id}`} rel="noopener noreferrer">
                <Image priority={true} className='group-hover:opacity-50 rounded-lg' layout='responsive' src={`${BASE_URL}${result.backdrop_path || result.poster_path}` || `${BASE_URL}${result.poster_path}`} alt='' height={1080} width={1920}/>
