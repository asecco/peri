...
function Episodes({result}) {
    ...
    return (
        <div className='p-2 group cursor-pointer transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50' onMouseEnter={e => { setHoverInfo({display: 'block'});}} onMouseLeave={e => { setHoverInfo({display: 'none'})}}>
            ...
            <div style={hoverInfo} layout='responsive' className="absolute bottom-20 p-4 group-hover:font-bold text-lg">
                <p className='line-clamp-7'>{result.overview || result.description}</p>
            </div>
            ...
            <div className='p-2 text-center'>
                ...
                <h2 className='mt-1 text-md text-white transition-all duration-100 ease-in-out group-hover:font-bold truncate max-w-md'>{`Episode ${result.episode_number}`}</h2>
            </div>
        </div>
    );
}
