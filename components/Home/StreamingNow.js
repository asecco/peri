    ...
    return (
        <div>
            <div className="flex justify-center md:justify-start md:mt-8 items-center">
                <p className="font-bold text-white text-4xl lg:text-5xl mx-6 xl:mx-8">Streaming Now</p>
            ...
            <div className="grid grid-cols-2 ml-2 md:ml-4 md:grid-cols-3 lg:grid-cols-6 gap-6 my-10">
                {tv?.slice(0, 6)?.map((show, index) => (
                    <div key={show?.id} className="grid grid-cols-2 md:flex items-center ml-6">
                        ...
                        <p className="text-white font-black mx-auto md:mx-2 text-5xl md:text-6xl lg:text-7xl 3xl:text-9xl">{index + 1}</p>
                    </div>
                ))}
            </div>
        </div>
