...
function Seasons({result, id}) {
    ...
    const routeQuery = () => router.push({pathname: '/episodeslist', query: {showId: id, season: result.season_number, overview: result.overview, poster: result.poster_path, air_date: result.air_date}}, `/${id}/season/${result.season_number}`);

    return (
        <div onClick={routeQuery} className='p-2 mx-20 md:mx-8 lg:mx-2 group cursor-pointer transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50'>
