...
function Seasons({result, id}) {
    ...
    const routeQuery = () => router.push({pathname: '/EpisodeList', query: {showId: id, season: result.season_number, overview: result.overview, poster: result.poster_path}}, `/EpisodeList/${id}/season/${result.season_number}`);
